...
# iterate over the moves list, then iterate over the move count to move boxes
for index, row in moves.iterrows():
    move_count = int(row['crate_count'])
    from_col = row['from_col']
    to_col = row['to_col']
    crate_count = len(crates[crates[from_col] != '-'])

    for i in range(move_count):
        # find target crate row based on offset, change that cell's value to -, append crate's value to the destination column
        crate = crates[from_col].iloc[crate_count - move_count + i ]
        crates[from_col].iloc[crate_count - move_count + i ] = '-'  
